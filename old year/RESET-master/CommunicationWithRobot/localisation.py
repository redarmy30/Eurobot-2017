...
class Robot(object):
	def __init__(self):
		...
		self.x = random.gauss(514, 5) 
		self.y = random.gauss(217, 5) 
		self.orientation = 0
		# Dummy sensor noise
		self.sense_noise = 50
...
if __name__ == '__main__':
	...
	# Set Robot randomly 
	myrobot = Robot()
	myrobot.x, myrobot.y, myrobot.orientation = 514.0, 951.0, -math.pi/6
	#print 'First: ', myrobot
	...
	#for iteration in xrange(30):
		...
		# Lidar sense - returns distance to 3 beacons
			angle, distance, x_glob, y_glob = ttest.update_di2(data_lidar, myrobot.pose()) 
			#print 'After lidar'			
			#try:
		...
		# 	print 'Final particles: ', i
			update_xy_plot([p[i].x for i in xrange(N)]+x_glob, [p[i].y for i in xrange(N)]+y_glob)	
			#update_xy_plot([p[i].x for i in xrange(N)], [p[i].y for i in xrange(N)])		
			end = time.time()
