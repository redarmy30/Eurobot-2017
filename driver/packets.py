...
def encode_params(params):
    ...
    for param in params:
        if isinstance(param, str):
            ...
            if not 0<= param <= 255:
                raise DriverException("Can't encode int: %d" % param)
            #res += chr(param)
            ...
            raise DriverException("Unexpected parameter type: %d" % type(param))
    return res

def encode_packet(cmd_id, params):
    enc_params = encode_params(params)
    msg = bytearray()
    msg += bytearray([SYNC, ADDR_SEND, 0, cmd_id]) + enc_params
    msg[2] = len(msg) + 2
    msg += cut_crc(crc(msg))
    return msg
...
def decode_params(cmd, params):
    ...
    return decode(params[:-1], 'ascii')  # x00 removed


def decode_packet(data):
    if data[0] != SYNC or data[1] != ADDR_RECV:
        raise DriverException('Wrong packet header: %s' % data)
    msg_len = data[2]
    if len(data) != msg_len:
        raise DriverException('Length missmatch, expected %d, got %d' %
            (msg_len, len(data)))
    cmd = data[3]
    if cmd not in REVERSED_CMD_LIST:
        raise DriverException('Unexpected command: %02x' % cmd)
    rev_cmd = REVERSED_CMD_LIST[cmd]
    msg_crc = cut_crc(crc(data[:-2]))
    if data[-2:] != msg_crc:
        raise DriverException('CRC missmatch')
    params = data[4:-2]
